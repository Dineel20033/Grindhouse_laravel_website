    ...
    <form action="{{ route('admin.products.store') }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="5" required
                      class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-amber-500 focus:border-amber-500">{{ old('description') }}</textarea>
        </div>

        <button type="submit" class="w-full bg-amber-600 text-white px-4 py-3 rounded-lg hover:bg-amber-700 font-semibold transition duration-300">
            Create Product
        </button>
    </form>
</div>
