...
file_path = r'c:\SSP laravel work\GrindHouse\resources\views\layouts\app.blade.php'
...
<body class="bg-gray-50 font-sans antialiased pb-24 md:pb-0">
    <div class="min-h-screen flex flex-col">
        ...
        <header class="px-6 md:px-12 bg-amber-100 shadow-lg relative z-50">
            <div class="flex items-center justify-between py-4">
                <a href="{{ route('home') }}" class="flex items-center">
                    <img src="{{ asset('assets/logo.png') }}" alt="Logo" class="w-20 md:w-28">
                </a>
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <a href="{{ route('cart.index') }}" class="relative inline-block">
                        <img src="{{ asset('assets/cart.png') }}" alt="Cart" class="w-7 h-7 md:w-8 md:h-8">
                        @if($cartCount > 0)
                            <span
        ...
        <footer class="bg-[#FFFBF0] py-8 md:py-12 text-gray-900 mt-auto">
            <div class="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-4 md:gap-8">
                {{-- Logo Section - Full width on very small screens, 1 col on others --}}
                <div class="col-span-2 sm:col-span-1 md:col-span-1 flex flex-col items-center md:items-start text-center md:text-left">
                    <h2 class="font-bold text-base md:text-lg mb-3 tracking-wider">GRINDHOUSE.LK</h2>
                    <img src="{{ asset('assets/logo.png') }}" alt="Logo" class="w-20 md:w-28 opacity-90 mix-blend-multiply">
                </div>

                {{-- About --}}
                <div class="col-span-1 text-center md:text-left flex flex-col items-center md:items-start">
                ...
                {{-- Links --}}
                <div class="col-span-2 sm:col-span-1 md:col-span-1 text-center md:text-left flex flex-col items-center md:items-start">
                    ...
                    <ul class="flex md:flex-col gap-4 md:gap-2 text-xs md:text-sm font-medium text-gray-700">
                        ...
                        <li><a href="{{ route('products.index') }}" class="hover:text-amber-700 transition">Products</a></li>
                        <li><a href="{{ route('about') }}" class="hover:text-amber-700 transition">About</a></li>
                        <li><a href="{{ route('contact.index') }}" class="hover:text-amber-700 transition">Contact</a></li>
                    </ul>
                </div>
            </div>
    ...
    <!-- Mobile Navigation -->
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 px-6 py-2 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
        <div class="flex justify-between items-end text-[10px] font-medium text-gray-500">
            <!-- Home -->
            <a href="{{ route('home') }}" class="flex flex-col items-center w-16 space-y-1 hover:text-amber-600 transition-colors {{ request()->routeIs('home') ? 'text-amber-600' : '' }}">
            ...
            <!-- Products -->
            <a href="{{ route('products.index') }}" class="flex flex-col items-center w-16 space-y-1 hover:text-amber-600 transition-colors {{ request()->routeIs('products.*') ? 'text-amber-600' : '' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Contact -->
            <a href="{{ route('contact.index') }}" class="flex flex-col items-center w-16 space-y-1 hover:text-amber-600 transition-colors {{ request()->routeIs('contact.*') ? 'text-amber-600' : '' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Auth -->
            @auth
                <a href="{{ route('profile.edit') }}" class="flex flex-col items-center min-w-[4rem] px-1 space-y-1 hover:text-amber-600 transition-colors {{ request()->routeIs('profile.*') ? 'text-amber-600' : '' }}">
            ...
            @else
                <a href="{{ route('login') }}" class="flex flex-col items-center w-16 space-y-1 hover:text-amber-600 transition-colors {{ request()->routeIs('login') ? 'text-amber-600' : '' }}">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                    <span>Sign In</span>
